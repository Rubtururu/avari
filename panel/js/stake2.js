...
function doStake() {
    if (user.balance / 1e18 < parseFloat($('.stake-inp-amount')[0].value)) {
        ...
        mainContract.methods.EnterStake(web3.utils.toWei($('.stake-inp-amount')[0].value,'ether'), parseInt($('.stake-inp-days')[0].value)).send({
            ...
            doAlert(`Successfully staked ${parseFloat($('.stake-inp-amount')[0].value)} AVC for ${parseInt($('.stake-inp-days')[0].value)} days.`, 3)
...
function doTotalRow() {
    ...
    `<tr role="row" class="" style="${bgRow}">
        ...
        <td aria-colindex="1" role="cell" class="" style="    padding: 8px 0px;border-top: 1px solid #ebe9f1; vertical-align: middle;">
            <div data-v-254b7dbb="" class="d-flex align-items-center">
                <div class="ltv-3-${133}" data-v-254b7dbb="" class="font-small-2 text-muted" style="font-weight:900; color: #24d8a4;">Total:</div>
            </div>
        </td>
        <td aria-colindex="1" role="cell" class="" style="    padding: 8px 0px;border-top: 1px solid #ebe9f1; vertical-align: middle;">
            <div data-v-254b7dbb="" class="d-flex align-items-center">
                <div class="ltv-4-${133}" data-v-254b7dbb="" class="font-small-2 text-muted">${(totaldata.staked).toFixed(3)}</div>
            </div>
        </td>
        <td aria-colindex="1" role="cell" class="" style="    padding: 8px 0px;border-top: 1px solid #ebe9f1; vertical-align: middle;">
            <div data-v-254b7dbb="" class="d-flex align-items-center">
...
function renderAStake(stake, id) {
    ...
    let progress
    let bonusTokens = (calcBonusToken(stake.endDay - stake.startDay, stake.tokenValue / 1e18)).toFixed(5)
...
function refreshGlobalData() {
    ...
    mainContract.methods.overall_stakedTokens().call({
        ...
        $('.gd-31')[0].innerHTML = ((parseInt(res)/1e18) - 162330).toFixed(2)
    })
    ...
    web3.eth.getBalance("0xeF8bca677118903bF8bbC512eDFFA1cbA6fDe4DC", function(err, result) {
        if (err) {
          console.log(err)
        } else {
...
$('.stake-inp-amount')[0].addEventListener('input', function (evt) {
    if ($('.stake-inp-days')[0].value) {
        $('.uns-11')[0].innerHTML = calcBonusToken($('.stake-inp-days')[0].value, this.value) + " AVC"
...
$('.stake-inp-days')[0].addEventListener('input', function (evt) {
    if ($('.stake-inp-amount')[0].value) {
        $('.uns-11')[0].innerHTML = calcBonusToken(this.value, $('.stake-inp-amount')[0].value) + " AVC"
        $('.uns-11')[0].style.color = "#000000b3"
    }
});
