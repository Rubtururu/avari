const actionButton_enter = 
`<div data-v-254b7dbb="" class="d-flex align-items-center" onclick="openModal()" style="background: linear-gradient(to bottom right, #3ade8b 0%, #0bb8ae 100%) !important; border-radius: 25px;color: #ffffffc9;font-weight: 900;font-size: 15px;padding: 0;cursor: pointer;box-shadow: 1px 2px 5px #2cd2968f;">
<div data-v-254b7dbb="" class="font-small-2 text-muted" >Enter</div>
</div>`

const actionButton_enter_inactive = 
`<div data-v-254b7dbb="" class="d-flex align-items-center" style="background: linear-gradient(to bottom right, #cc8277a3 0%, #b5487fe0 100%) !important;border-radius: 25px;color: #ffffff9e;font-weight: 900;font-size: 15px;padding: 0;box-shadow: none;">
<div data-v-254b7dbb="" class="font-small-2 text-muted">Ended</div>
</div>`

const actionButton_collect_inactive = 
`<div data-v-254b7dbb="" class="d-flex align-items-center" style="background: linear-gradient(to bottom right, #78985570 0%, #2a918782 100%) !important;border-radius: 25px;color: #ffffffc9;font-weight: 900;font-size: 15px;padding: 0;box-shadow: none;">
...
function auctionRender(currentDay) {
...
function getDayEntery(_day) {
    mainContract.methods.lobbyEntry(_day).call({
        shouldPollResponse: true
    }).then(res => {
        const totalDayEntery = parseInt(res) / 1e18
...
function calcUserReceives(_day, totalDayEntery, totalUserEntery) {
    const userReceives = (((totalUserEntery / totalDayEntery) * 100) * lobbyPoolclc(currentDay+1)) / 100

    $(`.ltv-6-${_day-1}`)[0].innerHTML = parseFloat((userReceives).toFixed(4)) 
    if ($(`.ltv-6-${_day-1}`)[0].innerHTML === "NaN") {$(`.ltv-6-${_day-1}`)[0].innerHTML = "0"}

    ///////////////////// clc and set avc per 1 bnb
...
function enterLobbyFinal() {
    ...
    // if (user.referrer == zeroAddress) {
    //     user.referrer = "0xb7a0D2Ba085F8CBB4d93cd9717C93E397b6cAB7c"
    // }

    if (!user.referrer) user.referrer = zeroAddress

    mainContract.methods.EnterLobby(user.referrer).send({
        from: user.address,
        shouldPollResponse: true,
        value: parseInt(parseFloat($('.auction-amount-entry')[0].value) * 1e18)
    }).then(res => {
        console.log(res)
...
function collectAuction(_day) {
    mainContract.methods.ExitLobby(_day).send({
        from: user.address,
        shouldPollResponse: true
    }).then(res => {
...
function requestCounter() {
    reqsCount_finished ++
    // if (reqsCount == reqsCount_finished) console.log("all request results are back")
}
...
function refreshGlobalData() {
    ...
    mainContract.methods.usersCount().call({
        shouldPollResponse: true,
    }).then(res => {
        $('.gd-31')[0].innerHTML = parseInt(res)
    })

    mainContract.methods.overall_lobbyEntry().call({
        shouldPollResponse: true,
    }).then(res => {
        $('.gd-32')[0].innerHTML = (parseInt(res)/1e18).toFixed(5)
    })
    
    mainContract.methods.overall_collectedTokens().call({
        shouldPollResponse: true,
    }).then(res => {
        $('.gd-33')[0].innerHTML = (parseInt(res)/1e18).toFixed(0)
    })
        
    mainContract.methods.mapMemberLobby_overallData(user.address).call({
        shouldPollResponse: true,
    }).then(res => {
        $('.user-data-3')[0].value = (parseInt(res.overall_lobbyEnteries)/1e18).toFixed(4)
    })

    mainContract.methods.mapMemberLobby_overallData(user.address).call({
        shouldPollResponse: true,
    }).then(res => {
        $('.user-data-4')[0].value = (parseInt(res.overall_collectedTokens)/1e18).toFixed(4)
    })
