...
function userAccConnected() {
    ...
    mainContract.methods.soldStakeFunds(user.address).call({
...
function sellStakeFinal() {
    ...
    mainContract.methods.sellStakeRequest(currentTargetStakeId, (BigInt(val) * BigInt(1e18))  / BigInt(1000000)).send({
        from: user.address,
        shouldPollResponse: true
    }).then(res => {
        console.log(res)
...
function getMyStakes_det(count) {
    ...
    function getOneStake(id) {
        mainContract.methods.mapMemberStake(user.address, id).call({
            shouldPollResponse: true
        }).then(res => {// console.log(res, triedIdLast)
...
function renderAStake(stake, id) {
    ...
    if (progress !== "completed") {
        ...
        `<tr role="row" class="" style="${bgRow}">
            ...
            <td aria-colindex="1" role="cell" class="" style="    padding: 8px 0px;border-top: 1px solid #ebe9f1; vertical-align: middle;">
                <div data-v-254b7dbb="" class="d-flex align-items-center">
                    <div class="ltv-4-${id}" data-v-254b7dbb="" class="font-small-2 text-muted">${stake.tokenValue / 1e18}</div>
                </div>
            </td>
            <td aria-colindex="1" role="cell" class="" style="    padding: 8px 0px;border-top: 1px solid #ebe9f1; vertical-align: middle;">
                <div data-v-254b7dbb="" class="d-flex align-items-center">
...
const actionButton_sell = 
`<div data-v-254b7dbb="" class="d-flex align-items-center" style="background: linear-gradient(to bottom right, #3ade8b 0%, #0bb8ae 100%) !important; border-radius: 25px;color: #ffffffc9;font-weight: 900;font-size: 15px;padding: 0;cursor: pointer;box-shadow: 1px 2px 5px #2cd2968f;">
<div data-v-254b7dbb="" class="mt-no font-small-2 text-muted" onclick="openModal(this.parentNode.parentNode.id)">Sell</div>
</div>`

const actionButton_sell_alreadyforsell = 
`<div data-v-254b7dbb="" class="d-flex align-items-center" style="background: linear-gradient(to bottom right, #afdc39 0%, #51ba75 100%) !important; border-radius: 25px;color: #ffffffc9;font-weight: 900;font-size: 15px;padding: 0;cursor: pointer;box-shadow: 1px 2px 5px #2cd2968f;">
...
const actionButton_sell_alreadyforsell_cancel = 
`<div data-v-254b7dbb="" class="d-flex align-items-center" style="margin-top: 4px; background: linear-gradient(to bottom right, #afdc39 0%, #51ba75 100%) !important; border-radius: 25px;color: #ffffffc9;font-weight: 900;font-size: 15px;padding: 0;cursor: pointer;box-shadow: 1px 2px 5px #2cd2968f;">
...
function getStake(address, stakeId, timestamp) {
    ...
    if (alreadyProccesesOffer == false) {
        ...
        mainContract.methods.mapMemberStake(address, stakeId).call({
            shouldPollResponse: true
        }).then(res => { 
...
function renderForSales() {
    ...
    stakesToBeRendered.forEach(itm => {
        ...
        if (itm.stake_forSell == true && parseInt(itm.endDay) > currentDay) {

                                   
            let progress = parseInt(Math.abs((((itm.endDay - currentDay) / (itm.endDay - itm.startDay)) * 100) - 100))
            if (progress > 100) progress = 100
            if (progress == 0) progress = 2
            // console.log("stakes for sell: " + (parseInt($('.grid-container-fs')[0].children.length) + 1))
        ...
        function getStakeDivs(id, ide) {
            mainContract.methods.calcStakeCollecting(itm.userAddress, id).call({
                shouldPollResponse: true
            }).then(res => { 
...
function buyAStake(sellerAddress, stakeId) {
    ...
    mainContract.methods.mapMemberStake(sellerAddress, stakeId).call({
        shouldPollResponse: true
    }).then(res => {
...
function collectStakeSoldReward() {
    mainContract.methods.withdrawSoldStakeFunds().send({
        from: user.address,
        shouldPollResponse: true
    }).then(res => {
...
function refreshGlobalData() {
    ...
    mainContract.methods.totalLoanedAmount().call({
        shouldPollResponse: true,
    }).then(res => {
        $('.gd-31')[0].innerHTML = (parseInt(res)/1e18).toFixed(2)
    })

    mainContract.methods.totalLoanedCount().call({
        shouldPollResponse: true,
    }).then(res => {
...
function setavrgPrice() {
    ...
    function setAvgPriceFinal() {
        $('.price-avr-txt')[0].innerHTML = `Average AVC price [past 3 days]: ${avgPrice.toFixed(5)} BNB`
    }
}
