...
let contractAddress = "0xFF2e3058E0C4732418Eb3d4C382328718C6c2Bdf"
...
function renderMoralisData(data, noattribute) {
	...
	if (noattribute) {
		...
		if (data[0]) {
			data[0].forEach(el => {
				txt = `Stake Started: ${parseInt(el.rawAmount) / 1e18} AVC for ${el.duration} days`
				dores(el)
			})
		}
...
function debug_setCurrentDay(_day) {
    mainContract.methods.debug_setCurrentDay(_day).send({
        from:user.address,
        shouldPollResponse: true
    })
}
